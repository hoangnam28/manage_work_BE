...
async function sendMailMaterialCore(subject, html, toList) {
...
function generateCreateMaterialEmailHTML(materialData, createdRecords) {
  ...
  const getStatusTitle = (status) => {
    switch(status) {
      case 'Pending':
        return '📝 Yêu cầu cập nhật Material Core';
      case 'Approve':
        return '✅ Đã cập nhật Material Core';
      case 'Cancel':
        return '❌ Hủy yêu cầu Material Core';
      default:
        return '🆕 Thông báo tạo mới Material Core';
    }
  };
  ...
  const recordsHTML = createdRecords.map((record, index) => `
        <tr>
            ...
            <td style="border: 1px solid #ddd; padding: 8px;">${record.nominal_thickness || 'N/A'}</td>
            <td style="border: 1px solid #ddd; padding: 8px;">${record.spec_thickness || 'N/A'}</td>
            <td style="border: 1px solid #ddd; padding: 8px;">${record.top_foil_cu_weight || 'N/A'}</td>
            <td style="border: 1px solid #ddd; padding: 8px;">${record.bot_foil_cu_weight || 'N/A'}</td>
            <td style="border: 1px solid #ddd; padding: 8px;">${record.center_glass || 'N/A'}</td>
  ...
  return `
        <div style="font-family: Arial, sans-serif; max-width: 800px; margin: 0 auto;">
            ...
            <div style="background-color: #f8f9fa; padding: 15px; border-radius: 5px; margin: 15px 0;">
                ...
                <table style="width: 100%; border-collapse: collapse;">
                    ...
                    <tr>
                        ...
                        <td>${formatDate(materialData.request_date)}</td>
                    </tr>
                    <tr>
                ...
                <table style="width: 100%; border-collapse: collapse; margin-top: 10px;">
                    <thead>
                        <tr style="background-color: #2c5aa0; color: white;">
                            ...
                            <th style="border: 1px solid #ddd; padding: 10px; text-align: left;">Nominal Thickness</th>
                            <th style="border: 1px solid #ddd; padding: 10px; text-align: left;">Spec Thickness</th>
                            <th style="border: 1px solid #ddd; padding: 10px; text-align: left;">Top Foil Cu Weight</th>
                            <th style="border: 1px solid #ddd; padding: 10px; text-align: left;">Bot Foil Cu Weight</th>
                            <th style="border: 1px solid #ddd; padding: 10px; text-align: left;">Center Glass</th>
            ...
            <div style="margin-top: 20px; text-align: center;">
                <a href="http://192.84.100.114:4000/material-core"
...
function generateStatusUpdateEmailHTML(materialId, oldStatus, newStatus, updatedBy, materialInfo = {}) {
  const getStatusInfo = (status) => {
    switch(status) {
      case 'Pending':
        return {
          title: '📝 Yêu cầu cập nhật Material Core',
          color: '#ffc107',
          icon: '📝'
        };
      case 'Approve':
        return {
          title: '✅ Đã cập nhật Material Core',
          color: '#28a745',
          icon: '✅'
        };
      case 'Cancel':
        return {
          title: '❌ Hủy yêu cầu Material Core',
          color: '#dc3545',
          icon: '❌'
        };
      default:
        return {
          title: '⏳ Cập nhật trạng thái Material Core',
          color: '#6c757d',
          icon: '⏳'
        };
    }
  ...
  // ✅ FIX: Format thời gian chính xác theo múi giờ Việt Nam
        ...
        <div style="font-family: Arial, sans-serif; max-width: 800px; margin: 0 auto;">
            ...
            <div style="background-color: #f8f9fa; padding: 15px; border-radius: 5px; margin: 15px 0;">
                ...
                <table style="width: 100%; border-collapse: collapse;">
                    ...
                    <tr>
                        <td style="padding: 5px 0; font-weight: bold;">Trạng thái cũ:</td>
                        <td><span style="background-color: #6c757d; color: white; padding: 3px 8px; border-radius: 3px; font-size: 12px;">${oldStatus}</span></td>
                    </tr>
                    <tr>
...
// Hàm tạo nội dung HTML cho email thông báo thay đổi dữ liệu material đã approve
        ...
        <div style="font-family: Arial, sans-serif; max-width: 800px; margin: 0 auto;">
            ...
            <div style="background-color: #fff; border: 1px solid #dee2e6; padding: 15px; border-radius: 5px; margin: 15px 0;">
                ...
                <table style="width: 100%; border-collapse: collapse;">
                    ...
                    ` : ''}
                    ${materialInfo.nominal_thickness ? `
                    <tr>
                        <td style="padding: 5px 0; font-weight: bold;">Nominal Thickness:</td>
                        <td>${materialInfo.nominal_thickness}</td>
                    </tr>
                    ` : ''}
                </table>
            </div>
